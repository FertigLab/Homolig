...
class Array(numpy.ndarray):
    ...
    def __new__(cls, alphabet=None, dims=None, data=None, dtype=float):
        ...
        if isinstance(data, dict):
            ...
            alphabet = "".join(alphabet) if single_letters else tuple(alphabet)
            n = len(alphabet)
            if dims == 1:
                shape = (n,)
            elif dims == 2:
        ...
        obj = super().__new__(cls, shape, dtype)
        obj[:] = 0.0 if data is None else data
        obj._alphabet = alphabet
        return obj
        ...
        if value.ndim == 2:
            if self.ndim == 2:
                ...
                if value.shape[0] == length and value.shape[1] == 1:
                    pass
                elif value.shape[0] != 1 or value.shape[1] != length:
                    raise IndexError("Requesting truncated array")
        elif value.ndim == 1:
    ...
    def __array_prepare__(self, out_arr, context=None):
        ...
        for arg in inputs:
            if isinstance(arg, Array) and arg.alphabet != alphabet:
    ...
    def copy(self):
        ...
        return Array(alphabet=self._alphabet, data=self)
    ...
    def format(self, fmt=""):
        ...
        """
        if fmt == "":
            fmt = "%i" if numpy.issubdtype(self.dtype, numpy.integer) else "%.1f"
        n = len(self.shape)
        if n == 1:
